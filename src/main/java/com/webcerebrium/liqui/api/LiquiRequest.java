...
public class LiquiRequest {
    ...
    public LiquiRequest(String requestUrl) throws LiquiApiException {

        this.requestUrl = requestUrl;
        try {
            url = new URL(requestUrl);
        } catch (MalformedURLException e) {
            throw new LiquiApiException("Mailformed URL " + e.getMessage());
        }
     ...
     */
    public LiquiRequest connect() throws LiquiApiException {
        ...
        try {
            ...
            throw new LiquiApiException("SSL Error " + e.getMessage() );
        } catch (KeyManagementException e) {
            throw new LiquiApiException("Key Management Error " + e.getMessage() );
        }

        try {
            conn = (HttpsURLConnection)url.openConnection();
        } catch (IOException e) {
            throw new LiquiApiException("HTTPS Connection error " + e.getMessage());
        }

        try {
            conn.setRequestMethod(method);
        } catch (ProtocolException e) {
            throw new LiquiApiException("HTTP method error " + e.getMessage());
        }
     ...
     * @throws LiquiApiException in case of any error
     */
    public LiquiRequest read() throws LiquiApiException {
        if (conn == null) {
            connect();
        }
        try {
            ...
            if (conn.getResponseCode() >= HttpURLConnection.HTTP_BAD_REQUEST) {
                ...
                if (obj.has("code") && obj.has("msg")) {
                    throw new LiquiApiException("ERROR: " +
                            obj.get("code").getAsString() + ", " + obj.get("msg").getAsString() );
                }
            }
        } catch (IOException e) {
            throw new LiquiApiException("Error in reading response " + e.getMessage());
        }
        return this;
    }

    public LiquiRequest payload(JsonObject payload) {
